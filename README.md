...
Some MobileWorld tasks require reaching **Google services** (Maps, Chrome's network stack, Play-services-backed APIs) and **github.com** from inside the emulator. If your host can't reach those directly, pass `--http-proxy`:

```bash
sudo uv run mw env run --count 1 --http-proxy http://proxy.host:8888
```

This wires the proxy into both the in-container Python stack and the Android emulator's system-wide proxy. Loopback and emulator-internal traffic (Mattermost, Mastodon, etc.) is automatically excluded.

> **⚠️ Requires Docker image v1.4 or newer.** Run `sudo uv run mw env check` to verify; it will offer to pull if your image is stale. Older images don't re-apply the Android proxy after task snapshot reverts, so the emulator silently bypasses your proxy after the first task.
